...
object Problem11 {
	val values: Array[Array[Int]] = Array(
  ...
  val maxHorizontalProduct = (for (y <- 0 to ydim - 1; x <- 0 to xdim - 4)
    yield (values(y)(x) * values(y)(x+1) * values(y)(x+2) * values(y)(x+3))).max
  val maxVerticalProduct = (for (y <- 0 to ydim - 4; x <- 0 to xdim - 1)
    yield (values(y)(x) * values(y+1)(x) * values(y+2)(x) * values(y+3)(x))).max
  val maxDiag1Product = (for (y <- 0 to ydim - 4; x <- 0 to xdim - 4)
    yield (values(y)(x) * values(y+1)(x+1) * values(y+2)(x+2) * values(y+3)(x+3))).max
  val maxDiag2Product = (for (y <- 0 to ydim - 4; x <- 0 to xdim - 4)
